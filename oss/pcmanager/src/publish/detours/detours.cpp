...
static DWORD __dwHookType = hookTypeInlineJmp;

DWORD DetourGetHookType()
{
	return __dwHookType;
}

void DetourSetHookType(DWORD dwHookType)
{
	__dwHookType = dwHookType;
}
...
{
    PBYTE pbJmpSrc = pbCode + SIZE_OF_JMP;
...
inline PBYTE detour_gen_call_immediate(PBYTE pbCode, PBYTE pbJmpVal)
{
	PBYTE pbJmpSrc = pbCode + SIZE_OF_JMP;

	*pbCode++ = 0xE8;   // jmp +imm32
	*((INT32*&)pbCode)++ = (INT32)(pbJmpVal - pbJmpSrc);
	return pbCode;
}
...
#error Feature not supported in this release.
			...
			{
			default:
			case hookTypeInlineJmp:
				pbCode = detour_gen_jmp_immediate(o->pbTarget, o->pTrampoline->pbDetour);
				break;

			case hookTypeInlineCall:
				pbCode = detour_gen_call_immediate(o->pbTarget, o->pTrampoline->pbDetour);
				break;
			}
