...
//#include "modifyprivilege/AccessHelper.h"//修改注册表权限
		...
		if (ERROR_SUCCESS != RegQueryValueEx(hOpenKey, pszName, 0, &dwType , (LPBYTE)pszValue, &dwSize))
		{
			...
			{//多字节解析
				if (dwSize > cbSize)
				{
					...
					nRet = _GetValue_Mul_SZ(hOpenKey, pszName, pszBuf, dwType, dwSize);
					SAFE_DELETE_PTR(pszBuf);
				}
				else
				{
					nRet = _GetValue_Mul_SZ(hOpenKey, pszName, pszValue, dwType, cbSize);
				}
			}
			else
			{//解析非多字节字符
				nRet = _GetValue(hOpenKey, pszName, pszValue, dwType, cbSize, dwSize);
			}
//			nRet = 0;
		}
	}
...
HKEY CRegisterOperate::_OpenKey(HKEY hKey, LPCTSTR pszSubPath)
{
	...
	{
		CAccessHepler addAccess;
		addAccess.AddRegAccess(hKey, pszSubPath, KEY_READ|KEY_WRITE, TRUE);//提权
...
int CRegisterOperate::SetRegValue(HKEY hKey, LPCTSTR pszSubPath, LPCTSTR pszName, LPCTSTR pszValue, DWORD cbSize)
{
	...
	{
		CAccessHepler().AddRegAccess(hKey, pszSubPath, KEY_ALL_ACCESS);
		if (ERROR_SUCCESS == RegSetValueEx(hOpenKey, pszName, 0, REG_SZ, (LPBYTE)pszValue, cbSize))
			nRet = 0;
	}
...
int CRegisterOperate::SetBinaryRegValue(HKEY hKey, LPCTSTR pszSubPath, LPCTSTR pszName, LPBYTE pByteValue, DWORD cbSize)
{
	int nRet = -1;
