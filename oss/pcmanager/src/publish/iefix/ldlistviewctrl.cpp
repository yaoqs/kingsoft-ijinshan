...
CRITICAL_SECTION g_MouseMoveLock;
...
CLDListViewCtrl::CLDListViewCtrl(void)
{
	...
	m_hCheckBitmp = BkBmpPool::GetBitmap(IDB_KWS_BMP_LIST_CHECK);
	m_hNotifyHwnd = NULL;
	m_bUrlHand = TRUE;
	m_bExTxtHand = TRUE;
	...
	m_bNeedReleaseCapture = FALSE;
	m_bTitleCanClickExpand = FALSE;
	//InitializeCriticalSection(&g_MouseMoveLock);
}

CLDListViewCtrl::~CLDListViewCtrl(void)
{
	//DeleteCriticalSection(&g_MouseMoveLock);
	//m_arrImage.RemoveAll();
}
...
// 	LPLDListViewCtrl_Item pItemData = _GetItemData(nItem);
// 	if (ITEM_TYPE_TITLE == pItemData->_enumItemType)
// 		memDC.FillSolidRect(&rcItem, RGB(0xc8,0,0));
...
void CLDListViewCtrl::_DrawTitle(CDC& memDC, int nItem, LPLDListViewCtrl_Item pItemStruct)
{
	...
	if (pItemStruct->_arrSubItem.GetSize() > 0)
	{
		...
		if (TRUE == m_bTitleCanClickExpand)
		{
			pItemStruct->_rcExpand.right = rcText.right;
			pItemStruct->_rcExpand.bottom = rcText.bottom;
		}
	}
	else
	{
...
void CLDListViewCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	//EnterCriticalSection(&g_MouseMoveLock);
	...
	do 
	{
		...
		{
			if (_PointInExTxtRect(nSelect, point) >= 0)
			{
				LPLDListViewCtrl_Item pItemData = _GetItemData(nSelect);
				LPLDListViewCtrl_SubItem pSubItem = pItemData->_arrSubItem[0];
				if (FALSE == m_bExTxtHand &&
					(NULL != pItemData && FALSE == pSubItem->_bExTxtHand))
				{
					SetCursor(::LoadCursor(NULL, IDC_ARROW));
					break;
				}
	...
	//LeaveCriticalSection(&g_MouseMoveLock);
	return ;
}
...
//	CString strMsg(L"_PointInUrlRect = -1");
	do 
	{
		if (FALSE == m_bUrlHand)
			break;
...
// 	strMsg.Format(L"_PointInUrlRect, %d", nRet);
// 	OutPut(strMsg);
	return nRet;
}
...
// 	CString strMsg(L"_PointInExTxtRect = -1");
//  	if (FALSE == m_bExTxtHand)
//  		return nRet;
...
// 	strMsg.Format(L"_PointInExTxtRect, %d", nRet);
// 	OutPut(strMsg);
...
void CLDListViewCtrl::_OnLButtonUpOnAnExTxt(int nItem, int nIndexExTxt)
{
	...
	if (TRUE == ::IsWindow(m_hNotifyHwnd))
	{
		DWORD dwValue = MAKELPARAM(nIndexExTxt, m_dwCtrlID);
		DWORD dwItem = MAKELPARAM(nItem, 0);
		SendMessage(m_hNotifyHwnd, MSG_USER_LDLISTVIEWCTRL_LBP_EXTXT, dwItem, dwValue);
	}
}
...
{
	int nRet = -1;
//	CString strMsg(L"_PointInButtonRect = -1");

	do 
	{
		if (-1 == m_mapBtnRect.FindKey(nItem))
			break;
...
//	strMsg.Format(L"_PointInButtonRect, %d", nRet);
//	OutPut(strMsg);
	return nRet;
}
...
{
	int nRet = -1;
//	CString strMsg(L"_PointInCheckBox = -1");
	do 
	{
...
//	strMsg.Format(L"_PointInCheckBox, %d", nRet);
//	OutPut(strMsg);
	return nRet;
}
...
{
	int nRet = -1;
//	CString strMsg(L"_PointInExPandRect = -1");
	do 
	{
...
// 	strMsg.Format(L"_PointInExPandRect, %d", nRet);
// 
// 	OutPut(strMsg);
...
LRESULT CLDListViewCtrl::OnNcLButtonDown(WPARAM wParam, CPoint ptPos)
{
	m_bNeedReleaseCapture = TRUE;
 	SetMsgHandled(FALSE);
	return 0;
}

LRESULT CLDListViewCtrl::OnNcLButtonUp(WPARAM wParam, CPoint ptPos)
{	
	SetMsgHandled(FALSE);
	return 0;
}

BOOL CLDListViewCtrl::SetTitleCanClickExpand(BOOL bExpand /* = FALSE */)
{
	BOOL bFlag = m_bTitleCanClickExpand;
	m_bTitleCanClickExpand = bExpand;
	return bFlag;
}