...
protected:
	...
	void _init_text()
	{
		m_str_draw_text = TEXT("");
		if ( m_i_fp_cnt > 0 )
		{
			TCHAR* pszPadding = new TCHAR[m_i_fp_cnt * 2 + 1];
			memset(pszPadding, 32, m_i_fp_cnt * 2 );
	...
	bool get_text_pos( HDC hdc, LPPOINT p_point )
	{
		...
		shpstr str_left( m_str_draw_text.Left( i_text_pos++ ) );
		if( m_i_linespace == 1)
			clac_text_point( hdc, p_point, m_str_draw_text, &m_draw_rect, i_text_pos, m_i_linespace );
		else
			clac_text_point_line( hdc, p_point, str_left, &m_draw_rect, m_i_linespace );
		return true;
	}

	void clac_text_point_line( HDC hdc, LPPOINT p_point, LPCTSTR pszText, LPRECT p_rect, int i_linespace )
	{
		CSize size;
		if( GetTextExtentPoint32( hdc, pszText, _tcslen( pszText ), &size ) )
		{
			p_point->x = p_rect->left + size.cx;
			p_point->y = p_rect->top ;
		}
		return;
	}
