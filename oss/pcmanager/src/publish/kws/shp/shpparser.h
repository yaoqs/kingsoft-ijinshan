...
public:
	...
	BEGIN_BASE_CLASS_MAP()
		...
		ADD_TYPE_CLASS( SHP_BAR_ITEM_KEY,	shp_bar_item)

	END_BASE_CLASS_MAP()

	void prase_text( LPCTSTR p_text )
	{
	...
	bool set_item_top( LPCTSTR lp_str_id, int n_top )
	{
		shpstr str_id( lp_str_id);
		shp_dom_item* p_item = get_item_by_id( &str_id );
		assert( p_item );
	...
	int get_item_top( LPCTSTR lp_str_id )
	{
		shpstr str_id( lp_str_id);
		shp_dom_item* p_item = get_item_by_id( &str_id );
		assert( p_item );
	...
	void set_btn_bmp_obj( LPCTSTR lp_str_id, HBITMAP h_bmp, HBITMAP h_over_bmp , HBITMAP h_down_bmp , int i_visible = 1 )
	{
		shpstr str_id(lp_str_id);
		shp_dom_item*p_item = get_item_by_id( &str_id );
		if( p_item )
		{
			if ( _tcsicmp( p_item->get_str_type(), SHP_BTN_ITEM_KEY) == 0 )
			{
				...
				p_btn->set_bmp_obj( h_bmp, h_over_bmp, h_down_bmp );
	...
	void set_shape_bg( LPCTSTR lp_str_id, shpclr clr )
	{
		shpstr str_id(lp_str_id);
		shp_dom_item*p_item = get_item_by_id( &str_id );
		if( p_item )
	...
	void set_bar_pos( LPCTSTR lp_str_id, int i_pos )
	{
		shpstr str_id(lp_str_id);
		shp_dom_item*p_item = get_item_by_id( &str_id );
		if( p_item )
		{
			if ( _tcsicmp( p_item->get_str_type(), SHP_BAR_ITEM_KEY) == 0 )
			{
				shp_bar_item * p_bar = (shp_bar_item*)p_item;
				p_bar->set_pos( i_pos );
			}
			else
			{
	...
	void set_bar_bmp_obj( LPCTSTR lp_str_id, HBITMAP bmp_bg, HBITMAP bmp_bg_left, 
		HBITMAP bmp_bg_right, HBITMAP bmp_bar, HBITMAP bmp_bar_left, HBITMAP bmp_bar_right )
	{
		shpstr str_id(lp_str_id);
		shp_dom_item*p_item = get_item_by_id( &str_id );
		if( p_item )
		{
			...
			{
				shp_bar_item * p_bar = (shp_bar_item*)p_item;
				p_bar->set_bar_bmp_obj( bmp_bg, bmp_bg_left, bmp_bg_right, 
									bmp_bar, bmp_bar_left, bmp_bar_right );
			}
			else
			{
				assert( false );
	...
	void set_bar_show_text( LPCTSTR lp_str_id, int i_show_text )
	{
		shpstr str_id(lp_str_id);
		shp_dom_item*p_item = get_item_by_id( &str_id );
		if( p_item )
		{
			...
			{
				shp_bar_item * p_bar = (shp_bar_item*)p_item;
				p_bar->set_show_text( i_show_text );
			}
			else
			{
				assert( false );
	...
	LPCTSTR get_link_url( LPCTSTR lp_str_id )
	{
		shpstr str_id(lp_str_id);
		shp_dom_item*p_item = get_item_by_id( &str_id );
		if( p_item )
