...
struct CListAnimateIcon
{
	unsigned int	nImage;
	CImageList		ImageList;
};

struct CListProgress
{
	int				nItem;
	int				nSubItem;
	unsigned int	nMax;
	unsigned int	nPos;
	CRect			rc;
	COLORREF		crOuterOutsideLine;
	COLORREF		crOuterInsideLine;
	COLORREF		crOuterTop;
	COLORREF		crOuterBottom;
	COLORREF		crInnerOutsideLine;
	COLORREF		crInnerTop;
	COLORREF		crInnerBottom;
};
...
public:
	CListImpl()
	{
		...
		m_bScrollDown = FALSE;
		m_bDragColumn = FALSE;
		m_bHyperLinkUnderlineFont = TRUE;
...
protected:
	...
	BOOL m_bScrollDown;
	BOOL m_bDragColumn;
	BOOL m_bHyperLinkUnderlineFont;
	...
	CListProgress		m_Progress;


	bool		m_bHeaderBmp;
	CBitmap		m_bmpHeaderBg;
	CBitmap		m_bmpHeaderDiv;
	CBitmap		m_bmpSelect;
...
public:
	BOOL SubclassWindow( HWND hWnd, CRect rc )
	{
		T* pT = static_cast<T*>(this);

		if (CWindowImpl< CListImpl >::SubclassWindow( hWnd ))
		{
			MoveWindow(rc);
			pT->Initialise();
			return TRUE;
	...
	void MoveWindowEx(CRect rc)
	{
		...
		int nAdd = rc.Height() - rcClient.Height();

		m_VScrollBar.GetWindowRect(rc);
		ScreenToClient(rc);
		rc.bottom += nAdd;
		m_VScrollBar.MoveWindow(rc);
	...
	BOOL Initialise()
	{
		...
		if ( m_curDivider.LoadCursor( IDC_DIVIDER ) == NULL )
			return FALSE;
		if ( m_curHyperLink.LoadSysCursor( IDC_HAND ) == NULL )
			return FALSE;
		...
		rcClient.bottom -= 1;
		rcClient.left = rcClient.Width() - ::GetSystemMetrics(SM_CYVSCROLL);
		
		m_VScrollBar.Create(m_hWnd, rcClient, NULL, WS_VISIBLE | WS_CHILD | SBS_VERT);
	...
	BOOL LoadSettings()
	{
		...
		m_rgbHeaderBackground = RGB(225,238,255);//GetSysColor( COLOR_BTNFACE );
		m_rgbHeaderBorder = RGB(174,200,235);//GetSysColor( COLOR_3DHIGHLIGHT );
		m_rgbHeaderShadow = GetSysColor( COLOR_3DSHADOW );
		m_rgbHeaderText = RGB(0x0e, 0x5b, 0x01);
		...
		m_rgbSelectOuter = RGB(196, 242, 180);//RGB( 170, 200, 245 );
		m_rgbSelectInner = RGB(240, 255, 240);//RGB( 230, 250, 250 );
		m_rgbSelectTop = RGB(219, 255, 209);//RGB( 210, 240, 250 );
		m_rgbSelectBottom = RGB(196, 242, 180);//RGB( 185, 215, 250 );
	...
	void SetHeader(int nBg, int nDiv)
	{
		m_bHeaderBmp = true;
		m_bmpHeaderBg = AtlLoadGdiplusImage(nBg, _T("PNG"));
		m_bmpHeaderDiv = AtlLoadGdiplusImage(nDiv, _T("PNG"));

		BITMAP bmpInfo = { 0 };
		m_bmpHeaderBg.GetBitmap(&bmpInfo);
		m_nHeaderHeight = bmpInfo.bmHeight;
	}
	void SetSelectBmp( int nSelect )
	{
		m_bmpSelect = AtlLoadGdiplusImage( nSelect, _T("PNG"));
	}

	void SetListRow(int nId)
	{
		m_bRowBg = true;
		m_bmpRowBg = AtlLoadGdiplusImage(nId, _T("PNG"));
		BITMAP bmpInfo = { 0 };
		m_bmpRowBg.GetBitmap(&bmpInfo);
		m_nItemHeight = bmpInfo.bmHeight;
	}
	...
	BOOL GetItemRect( int nItem, int nSubItem, CRect& rcItem )
	{
		...
		UINT nItemFlags = pT->GetItemFlags( nItem, nSubItem );
		if ( nSubItem != NULL_SUBITEM && !(nItemFlags & ITEM_FLAGS_SINGLE_COLUMN))
		{
			CRect rcColumn;
	...
	int GetTopItem()
	{
		return (int)( m_VScrollBar.GetScrollPos() / m_nItemHeight );
	}
	...
	BOOL EnsureItemVisible( int nItem, int nSubItem = NULL_SUBITEM )
	{
		...
		if ( rcItem.top < rcClient.top || rcItem.bottom > rcClient.bottom )
		{
			...
			if ( nScrollItem != NULL_ITEM )
				m_VScrollBar.SetScrollPos( nScrollItem * m_nItemHeight );
		}
	...
	void ResetScrollBars( int nScrollBar = SB_BOTH, int nScrollPos = -1, BOOL bRecalc = TRUE )
	{
		...
		if ( ( nScrollBar == SB_BOTH || nScrollBar == SB_VERT ) && m_bShowVertScroll )
		{
			...
			if (m_VScrollBar.IsWindow() && infoScroll.nMax > 0)
			{
				m_VScrollBar.ShowScrollBar();
				m_bEnableVertScroll = m_VScrollBar.SetScrollInfo(&infoScroll, TRUE ) ? ( infoScroll.nMax > 0 ) : FALSE;			
			}
			else if (m_VScrollBar.IsWindow() && infoScroll.nMax <= 0)
			{
				m_VScrollBar.ShowScrollBar(FALSE);
			}
		}
	...
	void DragColumn()
	{
		...
		HFONT hOldFont = dcHeader.SelectFont( m_fntListFont );
	...
	BOOL GroupSelect( CPoint point )
	{
		...
		int nVertScroll = m_VScrollBar.GetScrollPos();
	...
	BOOL AutoScroll( CPoint point )
	{
		...
		if ( point.y < rcClient.top )
		{
			...
			int nAutoScroll = m_VScrollBar.GetScrollPos();
	...
	void NotifyParent( int nItem, int nSubItem, int nMessage, CPoint pt = (DWORD)0L)
	{
		T* pT = static_cast<T*>(this);
	...
	void OnDestroy()
	{
		...
		if ( !m_fntItemFont.IsNull() )
			m_fntItemFont.DeleteObject();
	...
	void OnVScroll( int nSBCode, short nPos, HWND hScrollBar )
	{
		...
		int nScrollPos = m_VScrollBar.GetScrollPos();
		...
		switch ( nSBCode )
		{
			...
			case SB_THUMBPOSITION:	{
										...
										// get 32-bit scroll position
										if ( !m_VScrollBar.GetScrollInfo( &infoScroll ) )
											return;
	...
	void OnLButtonDown( UINT nFlags, ::CPoint point )
	{
		...
		m_bButtonDown = TRUE;
		m_ptDownPoint = point;
		m_ptSelectPoint = CPoint( point.x + GetScrollPos( SB_HORZ ), point.y + m_VScrollBar.GetScrollPos() );
		...
		switch ( nIDEvent )
		{
			...
			case ITEM_ANIMATE_TIMER:
				if (IsWindowVisible())
				{
					...
					for ( int nItem = 0; nItem < pT->GetItemCount(); nItem++ )
					{
						for ( int nSubItem = 0; nSubItem < pT->GetColumnCount(); nSubItem++ )
						{
							if (pT->GetItemImageAnimate(nItem, nSubItem))
							{
								pT->SetAnimate(nItem, nSubItem);
								pT->InvalidateItem(nItem, nSubItem);
							}
						}
					}
				}
	...
	void DrawBkgnd( CDCHandle dcPaint )
	{
		...
		CPen pen;
		pen.CreatePen( PS_SOLID, 1, m_rgbHeaderBorder );

		HPEN hOldPen = dcPaint.SelectPen( pen );
	...
	void DrawHeader( CDCHandle dcPaint )
	{
		...
		//rcHeaderItem.left += 2;
		//rcHeaderItem.right -= 2;
		
//		HPEN hOldPen = dcPaint.SelectPen( penHighlight );
//		dcPaint.Rectangle( &rcHeader );
//		::SelectObject(dcPaint, hOldPen);
		...
		for ( int nColumn = 0, nColumnCount = GetColumnCount(); nColumn < nColumnCount; rcHeaderItem.left = rcHeaderItem.right, nColumn++ )
		{
			...
			nHeaderWidth += rcHeaderItem.Width() + 2;
			...
			if (m_bHeaderBmp)
			{
				...
				HBITMAP hOldBmp = dcCompat.SelectBitmap(m_bmpHeaderBg);
				BITMAP bmpInfo = { 0 };
				m_bmpHeaderBg.GetBitmap(&bmpInfo);
				dcPaint.StretchBlt(rcHeaderItem.left, (rcHeaderItem.Height() - bmpInfo.bmHeight) / 2, listColumn.m_nWidth, bmpInfo.bmHeight, dcCompat, 0, 0, bmpInfo.bmWidth, bmpInfo.bmHeight, SRCCOPY);
				dcCompat.SelectBitmap(hOldBmp);

				hOldBmp = dcCompat.SelectBitmap(m_bmpHeaderDiv);
				m_bmpHeaderDiv.GetBitmap(&bmpInfo);
				dcPaint.StretchBlt(listColumn.m_nWidth, (rcHeaderItem.Height() - bmpInfo.bmHeight) / 2, bmpInfo.bmWidth, bmpInfo.bmHeight, dcCompat, 0, 0, bmpInfo.bmWidth, bmpInfo.bmHeight, SRCCOPY);
				dcCompat.SelectBitmap(hOldBmp);
			}
			else if ( m_bShowThemed && !m_thmHeader.IsThemeNull() )
			...
			else
			{
				...
				HPEN hOldPen = dcPaint.SelectPen( penShadow );
			...
			HFONT hOldFont = dcPaint.SelectFont( m_fntListFont );
	...
	void DrawRoundRect( CDCHandle dcPaint, CRect& rcRect, COLORREF rgbOuter, COLORREF rgbInner )
	{
		...
		HPEN hOldPen = dcPaint.SelectPen( penBorder );
		HBRUSH hOldBrush = dcPaint.SelectBrush( bshInterior );
		...
		hOldPen = dcPaint.SelectPen( penInnerBorder );
	...
	void DrawItem( CDCHandle dcPaint, int nItem, CRect& rcItem )
	{
		...
		BOOL bControlFocus = TRUE;//( GetFocus() == m_hWnd || m_bEditItem );
		
		UINT nItemFlags = pT->GetItemFlags( nItem, 0);
		if (m_bRowBg && !(nItemFlags & ITEM_FLAGS_NO_BACKGROUND))
		{
			CRect rc;
			GetClientRect( rc );
			...
			dcCompat.CreateCompatibleDC();
			// Stretch background over window client area
			HBITMAP hOldBmp = dcCompat.SelectBitmap(m_bmpRowBg);
			BITMAP bmpInfo = { 0 };
			m_bmpRowBg.GetBitmap(&bmpInfo);
			if (rcItem.top + 1 + bmpInfo.bmHeight >= rc.bottom)
			{
				dcPaint.StretchBlt(rcItem.left + 1, rcItem.top + 1, rcItem.Width(), rc.bottom - rcItem.top - 1 - 1, dcCompat, 0, 0, bmpInfo.bmWidth, bmpInfo.bmHeight, SRCCOPY);

			}
			else 
				dcPaint.StretchBlt(rcItem.left + 1, rcItem.top + 1, rcItem.Width(), bmpInfo.bmHeight, dcCompat, 0, 0, bmpInfo.bmWidth, bmpInfo.bmHeight, SRCCOPY);
		...
		if ( bSelectedItem )
		{
			if ( m_bShowThemed )  //&& !m_thmHeader.IsThemeNull() )
			{
				...
				if( !m_bmpSelect.IsNull() )
				{
					BITMAP infoSelect;
					CDC dcCompat;
					dcCompat.CreateCompatibleDC();
					HBITMAP hOldBmp = dcCompat.SelectBitmap( m_bmpSelect );
					m_bmpSelect.GetBitmap( &infoSelect );
					dcPaint.StretchBlt(rcItem.left + 1, rcItem.top + 1, rcItem.Width(), rcItem.Height(), dcCompat, 0, 0, infoSelect.bmWidth, rcItem.Height(), SRCCOPY);
					dcCompat.SelectBitmap(hOldBmp);
				}
				else
				{
					...
					rcSelect.left += 1;
					DrawRoundRect( dcPaint, rcSelect, bControlFocus ? m_rgbSelectOuter : m_rgbNoFocusOuter, bControlFocus ? m_rgbSelectInner : m_rgbNoFocusInner );
		...
		for ( int nSubItem = 0, nColumnCount = GetColumnCount(); nSubItem < nColumnCount; rcSubItem.left = rcSubItem.right + 1, nSubItem++ )
		{
			...
			if (!(nItemFlags & ITEM_FLAGS_SINGLE_COLUMN))
			...
			else if ( pT->GetItemColours( nItem, nSubItem, rgbBackground, rgbText ) && rgbBackground != m_rgbBackground )
			{
				...
				CRect rcBg(rcSubItem);
				CBrush brush( ::CreateSolidBrush(RGB(255, 255, 255)) );
				dcPaint.FrameRect( &rcBg, brush );

				rcBg.DeflateRect(1, 1);
				dcPaint.RoundRect(rcBg, CPoint( 3, 3 ) );
			...
			{
				rcItemText.left += rcItemText.Width() / 2 - pT->GetItemTextPos( nItem, listColumn.m_nIndex );
			}
			else if (nItemFlags & ITEM_FLAGS_RIGHT)
			{
				rcItemText.right -= pT->GetItemTextPos( nItem, listColumn.m_nIndex );
			}
			else
				rcItemText.left += pT->GetItemTextPos( nItem, listColumn.m_nIndex );
			
			// draw subitem image if supplied
			if ( !m_ilItemImages.IsNull() && ( !m_bEditItem || ( m_bEditItem && !bFocusSubItem ) ) )
			{
				CSize sizeIcon;
				if (pT->GetItemImageAnimate(nItem, nSubItem))
				{
					CListAnimateIcon* pAnimateIcon = pT->GetItemImageAnimateIcon(nItem, nSubItem);
					if (pAnimateIcon)
					{
						pAnimateIcon->ImageList.GetIconSize( sizeIcon );
						...
						pAnimateIcon->ImageList.DrawEx( pAnimateIcon->nImage, dcPaint, rcItemImage, CLR_DEFAULT, CLR_DEFAULT, ILD_TRANSPARENT );

						// offset item text (for image)
						rcItemText.left += sizeIcon.cx + 4;
					}
				}
				else if (nItemImage != ITEM_IMAGE_NONE)
				{
					m_ilItemImages.GetIconSize( sizeIcon );
			...
			HFONT hOldFont = dcPaint.SelectFont( pT->GetItemFont( nItem, nSubItem ) );
			//dcPaint.SetTextColor( ( ( !m_bShowThemed || m_thmHeader.IsThemeNull() ) && bSelectedItem && !bFocusSubItem ) ? m_rgbSelectedText : rgbText );
			dcPaint.SetTextColor(rgbText);
			dcPaint.SetBkMode( TRANSPARENT );

			UINT nFormat = DT_SINGLELINE | DT_NOPREFIX | DT_VCENTER | DT_END_ELLIPSIS;

			if ( (nItemFlags & ITEM_FLAGS_CENTRE) && pT->GetItemTextPos( nItem, listColumn.m_nIndex ) == 0)
				nFormat |= DT_CENTER;
			else if ( nItemFlags & ITEM_FLAGS_RIGHT )
				nFormat |= DT_RIGHT;
			else
			...
			switch ( nItemFormat )
			{
				...
				case ITEM_FORMAT_CHECKBOX_3STATE:	{
														...
														else
														{
															...
															if ( nCheckValue < 0 )
																...
																nCheckImage = ITEM_IMAGE_CHECK_ON;
				...
				case ITEM_FORMAT_PROGRESS:			if ( m_bShowThemed && !m_thmProgress.IsThemeNull() && !( nItemFlags & ITEM_FLAGS_PROGRESS_SOLID ) )
													...
													{
														if (m_Progress.nItem == nItem 
															&& (m_Progress.nSubItem == -1 || m_Progress.nSubItem == nSubItem))
														{
															CRect rcProgress( rcSubItem );
															rcProgress.DeflateRect( (rcSubItem.Width() - m_Progress.rc.Width()) / 2, (rcSubItem.Height() - m_Progress.rc.Height()) / 2);
															m_Progress.rc = rcProgress;

															CPen penOuterOutside;
															penOuterOutside.CreatePen( PS_SOLID, 1, m_Progress.crOuterOutsideLine );
															CBrush bshInterior;
															bshInterior.CreateSolidBrush( RGB(255, 255, 255) );

															HPEN hOldPen = dcPaint.SelectPen( penOuterOutside );
															HBRUSH hOldBrush = dcPaint.SelectBrush( bshInterior );

															dcPaint.RoundRect( rcProgress, CPoint( 2, 2) );
															dcPaint.SelectPen(hOldPen);
																
															
															rcProgress.DeflateRect(1, 1);
															CPen penOuterInside;
															penOuterInside.CreatePen( PS_SOLID, 1, m_Progress.crOuterInsideLine );
															hOldPen = dcPaint.SelectPen( penOuterInside );
															...
															rcProgress.DeflateRect(1, 1);
															CRect rcBg(rcProgress);
															rcBg.left = rcProgress.left + min(m_Progress.nPos, m_Progress.nMax) * rcProgress.Width() / m_Progress.nMax;
															DrawGradient( dcPaint, rcBg, m_Progress.crOuterTop, m_Progress.crOuterBottom );
															
															if (m_Progress.nPos != 0)
															{
																rcProgress.right = rcBg.left;
																CPen penInner;
																penInner.CreatePen( PS_SOLID, 1, m_Progress.crInnerOutsideLine );
																hOldPen = dcPaint.SelectPen( penInner );
																...
																rcProgress.DeflateRect(1, 1);
																DrawGradient( dcPaint, rcProgress, m_Progress.crInnerTop, m_Progress.crInnerBottom );
															}
				...
				default:							// draw item text
													...
													{
														CSize szText;
														dcPaint.GetTextExtent(strItemText, strItemText.GetLength(), &szText);
														dcPaint.DrawText( strItemText, strItemText.GetLength(), rcItemText, nFormat );
														dcPaint.SelectFont( hOldFont );



														strItemText = pT->GetItemText( nItem, listColumn.m_nIndex, true );
														if (!strItemText.IsEmpty())
														{
															hOldFont = dcPaint.SelectFont( pT->GetItemFont( nItem, nSubItem, true ) );
															pT->GetItemColours( nItem, nSubItem, rgbBackground, rgbText, true);
															dcPaint.SetTextColor(rgbText);
															rcItemText.left += szText.cx + 5;
															dcPaint.DrawText( strItemText, strItemText.GetLength(), rcItemText, nFormat );
														}
													}
													break;
	...
	void DrawSelect( CDCHandle dcPaint )
	{
		...
		dcGroupSelect.SetBkColor( m_rgbSelectedItem );
		...
		bshSelectFrame.CreateSolidBrush( m_rgbSelectedItem );
	...
	//为了更便捷控制，添加一些设置函数。
	void SetThemeShow(BOOL bShow)
	{
		m_bShowThemed = bShow;
	}
	//设置头部的背景色
	void SetHeaderBackground(COLORREF rgbHeaderBg)
	{
		m_rgbHeaderBackground = rgbHeaderBg;
	}
	//设置头部的文字颜色
	void SetHeaderTextColor(COLORREF rgbHeaderText)
	{
		m_rgbHeaderText = rgbHeaderText;
	}
	//设置HyperLink型的项，鼠标指上去时，是否变换字体
	void SetIsHyperLinkFont(BOOL bUnderline)
	{
		m_bHyperLinkUnderlineFont = bUnderline;
	}
...
struct CSubItem
{
	...
	int nPos;
	bool bAnimateImage;
	CString m_strSubText;
	HFONT m_hSubFont;
	COLORREF m_rgbSubText;
	CListAnimateIcon AnimateIcon;
};
...
public:
	int InsertItem(int nIndex, CListItem< TData >& listItem )
	{
	...
	int AddItem( LPCTSTR lpszText, int nImage = ITEM_IMAGE_NONE, UINT nFormat = ITEM_FORMAT_NONE, UINT nFlags = ITEM_FLAGS_NONE )
	{
		...
		listSubItem.m_rgbText = m_rgbItemText;
		listSubItem.nPos = 0;
		listSubItem.bAnimateImage = false;
		...
		// set item details for first subitem
		if (listItem.m_aSubItems.GetSize() > 0)
		{
			listItem.m_aSubItems[ 0 ].m_strText = lpszText;
			listItem.m_aSubItems[ 0 ].m_nImage = nImage;
		}
	...
	CString GetItemText( int nItem, int nSubItem, bool bSub = false )
	{
		CSubItem listSubItem;
		if (!bSub)
			return GetSubItem( nItem, nSubItem, listSubItem ) ? listSubItem.m_strText : _T( "" );
		else
			return GetSubItem( nItem, nSubItem, listSubItem ) ? listSubItem.m_strSubText : _T( "" );
	}

	int GetItemTextPos( int nItem, int nSubItem )
	{
		CSubItem listSubItem;
		return GetSubItem( nItem, nSubItem, listSubItem ) ? listSubItem.nPos : 0;
	}
	...
	bool GetItemImageAnimate( int nItem, int nSubItem )
	{
		CSubItem listSubItem;
		return GetSubItem( nItem, nSubItem, listSubItem ) ? listSubItem.bAnimateImage : false;
	}

	CListAnimateIcon* GetItemImageAnimateIcon( int nItem, int nSubItem )
	{
		if ( nItem < 0 || nItem >= GetItemCount() ) 
			return FALSE;
		...
		if (m_aItems[ nItem ].m_aSubItems[ nSubItem ].bAnimateImage)
		{
			return &m_aItems[ nItem ].m_aSubItems[ nSubItem ].AnimateIcon;
		}
		else
			return NULL;
	}
	...
	HFONT GetItemFont( int nItem, int nSubItem, bool bSub = false )
	{
		CSubItem listSubItem;
		if ( !GetSubItem( nItem, nSubItem, listSubItem ) )
			return FALSE;
		...
		if (!bSub)
			...
			return listSubItem.m_hSubFont == NULL ? CListImpl< CListCtrlData >::GetItemFont( nItem, nSubItem ) : listSubItem.m_hSubFont;
	}
	
	BOOL GetItemColours( int nItem, int nSubItem, COLORREF& rgbBackground, COLORREF& rgbText, bool bSub = false )
	{
		CSubItem listSubItem;
		if ( !GetSubItem( nItem, nSubItem, listSubItem ) )
			return FALSE;
		...
		if (!bSub)
			...
			rgbText = listSubItem.m_rgbSubText;
	...
	BOOL SetItemText( int nItem, int nSubItem, LPCTSTR lpszText, bool bSub = false)
	{
		if ( nItem < 0 || nItem >= GetItemCount() ) 
			return FALSE;
		if ( nSubItem < 0 || nSubItem >= (int)m_aItems[ nItem ].m_aSubItems.GetSize() )
		...
		if (!bSub)
			...
			m_aItems[ nItem ].m_aSubItems[ nSubItem ].m_strSubText = lpszText;

		return TRUE;
	}

	BOOL SetItemTextPos(int nItem, int nSubItem, int nPos)
	{
		if ( nItem < 0 || nItem >= GetItemCount() ) 
			return FALSE;
		if ( nSubItem < 0 || nSubItem >= (int)m_aItems[ nItem ].m_aSubItems.GetSize() )
			return FALSE;
		m_aItems[ nItem ].m_aSubItems[ nSubItem ].nPos = nPos;
		return TRUE;
	}
	...
	BOOL SetItemImage( int nItem, int nSubItem, int nImage )
	{
		...
		m_aItems[ nItem ].m_aSubItems[ nSubItem ].bAnimateImage = false;
		return TRUE;
	}

	BOOL SetItemAnimateIcon( int nItem, int nSubItem, int nResID, int cx)
	{
		if ( nItem < 0 || nItem >= GetItemCount() ) 
			return FALSE;
		if ( nSubItem < 0 || nSubItem >= (int)m_aItems[ nItem ].m_aSubItems.GetSize() )
		...
		m_aItems[ nItem ].m_aSubItems[ nSubItem ].bAnimateImage = true;
		if (m_aItems[ nItem ].m_aSubItems[ nSubItem ].AnimateIcon.ImageList.IsNull())
		{
			m_aItems[ nItem ].m_aSubItems[ nSubItem ].AnimateIcon.nImage = 0;
			m_aItems[ nItem ].m_aSubItems[ nSubItem ].AnimateIcon.ImageList.CreateFromImage(nResID, cx, 0, RGB( 255, 255, 255 ), IMAGE_BITMAP, LR_CREATEDIBSECTION );
			SetTimer( ITEM_ANIMATE_TIMER, 150 );
		}

		return TRUE;
	}

	BOOL SetAnimate(int nItem, int nSubItem)
	{
		if ( nItem < 0 || nItem >= GetItemCount() ) 
			return FALSE;
		if ( nSubItem < 0 || nSubItem >= (int)m_aItems[ nItem ].m_aSubItems.GetSize() )
		...
		if (++m_aItems[ nItem ].m_aSubItems[ nSubItem ].AnimateIcon.nImage >= m_aItems[ nItem ].m_aSubItems[ nSubItem ].AnimateIcon.ImageList.GetImageCount())
			m_aItems[ nItem ].m_aSubItems[ nSubItem ].AnimateIcon.nImage = 0;
	}

	BOOL SetItemProgress(CListProgress& Progress)
	{
		m_Progress = Progress;
		return TRUE;
	}

	void SetItemProgressPos(unsigned nPos)
	{
		if (m_Progress.nPos < nPos)
		{
			m_Progress.nPos = nPos;
			InvalidateRect(m_Progress.rc);
		}
	}
	...
	BOOL SetItemFont( int nItem, int nSubItem, HFONT hFont, bool bSub = false)
	{
		if ( nItem < 0 || nItem >= GetItemCount() ) 
			return FALSE;
		if ( nSubItem < 0 || nSubItem >= (int)m_aItems[ nItem ].m_aSubItems.GetSize() )
		...
		if (!bSub)
			...
			m_aItems[ nItem ].m_aSubItems[ nSubItem ].m_hSubFont = hFont;
	...
	void SetItemFont()
	{
		...
		m_fntItemFont.CreateFontIndirect(&lf);
	}
	...
	void SetListFont()
	{
		...
		m_fntListFont.CreateFontIndirect(&lf);
	}

	BOOL SetItemColours( int nItem, int nSubItem, COLORREF rgbBackground, COLORREF rgbText, bool bSub = false)
	{
		if ( nItem < 0 || nItem >= GetItemCount() )
			return FALSE;
		if ( nSubItem < 0 || nSubItem >= (int)m_aItems[ nItem ].m_aSubItems.GetSize() )
		...
		if (!bSub)
			...
			m_aItems[ nItem ].m_aSubItems[ nSubItem ].m_rgbSubText = rgbText;
	...
	{
	public:
		CompareItem( int nColumn, BOOL bAscending = TRUE) : m_nColumn( nColumn ), m_bAscending(bAscending) {}
	...
	void SortItems( int nColumn, BOOL bAscending )
	{
		m_aItems.Sort( CompareItem( nColumn, bAscending ) );
	}
