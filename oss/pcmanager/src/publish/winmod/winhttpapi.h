...
public:
	...
	BOOL Uninitialize();
public:
	HINTERNET OpenSession(LPCWSTR pszAgentW, DWORD dwAccessType, LPCWSTR pszProxyW, LPCWSTR pszProxyBypassW, DWORD dwFlags);
	HINTERNET OpenSession(LPCWSTR userAgent /* = NULL */, LPCTSTR pszHostPort);
	...
	BOOL PrepareReceiveResponse(HINTERNET hRequest);
