...
//////////////////////////////////////////////////////////////////////////
//
HINTERNET WinHttpApi::OpenSession(LPCWSTR pszAgentW, DWORD dwAccessType, LPCWSTR pszProxyW, LPCWSTR pszProxyBypassW, DWORD dwFlags)
{
	HINTERNET hSession = NULL;

	if (bUseWinHttp_)
		hSession = (WinHttpApi_.pWinHttpOpen)(pszAgentW, dwAccessType, pszProxyW, pszProxyBypassW, dwFlags/*WINHTTP_FLAG_ASYNC*/);
	else
		hSession = (WinINetApi_.pInternetOpen)(pszAgentW, dwAccessType, pszProxyW, pszProxyBypassW, dwFlags);
	
	return hSession;
}

HINTERNET WinHttpApi::OpenSession(LPCWSTR userAgent /* = NULL */, LPCTSTR pszHostPort)
{
	if (FALSE == bInited_)
		return NULL;
	...
	if (bUseWinHttp_)
		...
		hSession = (WinINetApi_.pInternetOpen)(userAgent, INTERNET_OPEN_TYPE_PROXY , pszHostPort, NULL, 0);
...
BOOL WinHttpApi::PrepareReceiveResponse(HINTERNET hRequest)
{
	BOOL bSuccess = FALSE;
...
BOOL WinHttpApi::SetTimeouts(IN HINTERNET hInternet, IN int nResolveTimeout, IN int nConnectTimeout, IN int nSendTimeout, IN int nReceiveTimeout)
{
	...
	{
        bSuccess = WinINetApi_.pInternetSetOption(hInternet, INTERNET_OPTION_CONNECT_TIMEOUT, &nConnectTimeout, sizeof(int));
        bSuccess = WinINetApi_.pInternetSetOption(hInternet, INTERNET_OPTION_SEND_TIMEOUT, &nSendTimeout, sizeof(int));
        bSuccess = WinINetApi_.pInternetSetOption(hInternet, INTERNET_OPTION_RECEIVE_TIMEOUT, &nReceiveTimeout, sizeof(int));
		//bSuccess = (WinINetApi_.pInternetReadFile)(hRequest, buffer, length, cbRead);
	}
...
BOOL WinHttpApi::WinHttpSetOption(HINTERNET hInternet, DWORD dwOption, LPVOID lpBuffer, DWORD dwBufferLength)
{
	if (FALSE == bInited_)
		return FALSE;
	...
	if (bUseWinHttp_)
	{
		bRet = (WinHttpApi_.pWinHttpSetOption)(hInternet, dwOption, lpBuffer, dwBufferLength);
	}
	else
	{
		bRet=(WinINetApi_.pInternetSetOption)(hInternet, dwOption, lpBuffer, dwBufferLength);
	}
