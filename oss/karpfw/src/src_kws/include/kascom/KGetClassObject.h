...
#define KAS_BEGIN_OBJECT_MAP()   HRESULT KDllModule_DllGetClassObject( REFIID riid, LPVOID* ppv ){ \
	if( ppv == NULL ) return E_INVALIDARG;


#define KAS_OBJECT_ENTRY( IXXX, CXXX )  if( riid == __uuidof( IXXX ) ){ \
...
#define KAS_OBJECT_ENTRY_IID( IXXX, CXXX )  if( riid == IID_##IXXX ) ){ \
	IXXX* pI = new KDllComObject<CXXX>; \
	if( pI == NULL ) return E_OUTOFMEMORY; \
	pI->AddRef(); \
	*ppv = pI; \
...
#define KAS_END_OBJECT_MAP()  	*ppv = NULL; \
	return E_NOINTERFACE; }
